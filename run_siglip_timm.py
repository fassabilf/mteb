...
# ==============================================================
# MONKEY-PATCH: Fix dataclasses on broken default_factory=typing.List
# ==============================================================
...
# # ------------------------------------------------------------------
# # Register the timm SigLIP ModelMeta so mteb.get_model() can find it.
# # Import from the adapted openclip_models file (same directory).
# # ------------------------------------------------------------------
# from openclip_models import siglip_ViT_SO400M_14_timm  # noqa: F401
...
# 2. MODEL + BENCHMARK
# ==============================================================
model_name = "timm/ViT-SO400M-14-SigLIP"
...
def is_cpu_fit_task(task) -> bool:
    try:
        return isinstance(task, AbsTaskClassification)
    except Exception:
        return False
...
# ==============================================================
# 3. Run task by task (REVERSED ORDER)
# ==============================================================
t0 = time.time()
completed = []
failed = []
...
for task in tasks:
    ...
    # ---- Aggregate: run subtasks one by one (REVERSED) ----
    if is_aggregate:
        print(f"\n--- Aggregate: {task_name} ---")
    ...
    # # ---- Normal task ----
    # if is_cpu_fit_task(task):
    #     print(f"\n--- SKIPPED (classification): {task_name} ---")
